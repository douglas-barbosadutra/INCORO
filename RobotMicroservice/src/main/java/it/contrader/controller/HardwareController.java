...
public class HardwareController {
	...
	public ResponseEntity<Boolean> deleteHardware(@RequestBody ParamDTO paramDTO) {			
		...
		try {
			...
			if(type == 1) {
				int idHardware = (int) paramDTO.getParam();
				return ResponseEntity.status(HttpStatus.OK).body(hardwareService.deleteHardware(idHardware));
			}
			else
	...
	@RequestMapping(value="/insertHardware", method= RequestMethod.POST)
	public ResponseEntity<HardwareDTO> insertLabel(@RequestBody ParamDTO paramDTO) {
		int rank;
		int idUser;
